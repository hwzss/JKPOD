...
- (void)initWindowView
{
    ...
    m_play = [[VideoPlay alloc]
        initWithRect:CGRectMake(0, super.m_yOffset, m_screenFrame.size.width,
                         m_screenFrame.size.width * 9 / 16)
               Index:0];
                         ...
                         [library assetForURL:assetURL
                             resultBlock:^(ALAsset* asset) {
                                 ...
                                 [NSThread
                                     detachNewThreadSelector:@selector(updateText:)
                                                    toTarget:self
                                                  withObject:@"Save Picture Successfully"];
                             }
                             failureBlock:^(NSError* error) {
                                 ...
                                 [NSThread
                                     detachNewThreadSelector:@selector(updateText:)
                                                    toTarget:self
                                                  withObject:@"Save Picture Failed"];
                             }];
                     }];
...
- (void)saveVideoToPhone:(NSString*)path
{
    ...
    if (url != nil) {
        PHFetchResult* result = [PHAsset fetchAssetsWithALAssetURLs:[NSArray arrayWithObject:url]
                                                            options:nil];
        if (result.count) {
            NSLog(@"录像已存在");
            NSMutableArray* toDeleteAssets = [NSMutableArray new];
                           ...
                           completionBlock:^(NSURL* assetURL, NSError* error) {
                               [library assetForURL:assetURL
                                   resultBlock:^(ALAsset* asset) {
                                       ...
                                       [NSThread detachNewThreadSelector:@selector(updateText:)
                                                                toTarget:self
                                                              withObject:@"Save Record Successfully"];
                                   }
                                   failureBlock:^(NSError* error) {
                                       NSLog(@"录像保存失败");
                                       [NSThread detachNewThreadSelector:@selector(updateText:)
              ...
              force:(BOOL)beForce
{
    ...
    if (UIInterfaceOrientationIsPortrait(InterfaceOrientation)) {
        ...
        [livePlayBarView setFrame:CGRectMake(0, super.m_yOffset - LIVE_BAR_HEIGHT + m_screenImg.frame.size.height,
                                      m_screenImg.frame.size.width, LIVE_BAR_HEIGHT)];
        [self refreshSubView];
        [m_fullScreenBtn setBackgroundImage:[UIImage imageNamed:@"videoplay_icon_fullscreen"]
                                   forState:UIControlStateNormal];
        super.m_navigationBar.hidden = NO;
    } else {
        [m_fullScreenBtn setBackgroundImage:[UIImage imageNamed:@"video_smallscreen"]
                                   forState:UIControlStateNormal];
