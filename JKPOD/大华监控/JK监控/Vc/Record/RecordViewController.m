...
- (void)onDownload:(UIButton*)sender
{
    ...
    [self reloadCell:m_listView Section:0 Row:m_index];
    [m_listViewLock unlock];
    [m_download startDownload:m_index filepath:m_downloadPath token:m_accessToken devID:m_strDevSelected psk:m_encryptKey recordID:recId Type:0 Timeout:10];
}
...
- (void)onDownloadState:(NSInteger)index code:(NSString*)code type:(NSInteger)type
{
    ...
    if (1 == type) {
        ...
        if ([HLS_Result_String(HLS_DOWNLOAD_TIMEOUT) isEqualToString:code]) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)downloadThread
{
    ...
    while (m_looping) {
        ...
        if (response == nil) {
            ...
            NSString* key = (m_encryptKey && 0 != m_encryptKey.length) ? m_encryptKey : m_strDevSelected;
            NSInteger iret = [m_util decryptPic:m_downloadPicture[m_downloadingPos].picData key:key bufOut:&dataOut];
